...
export default function ShareMenuModal({ isOpen, onClose, hotelId, hotelName, properties = [] }: ShareMenuModalProps) {
  ...
  const generateCanvas = async (targetLocValue: string): Promise<HTMLCanvasElement | null> => {
    ...
    // Decorative Primary/Purple glow top
    ...
    try {
        ...
        const cardY = cursorY + 100; // Use cursorY
        const r = 100;

        // --- Gradient Glow Effect ---
        // Simulating: bg-gradient-to-tr from-primary via-purple-500 to-indigo-500
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-xl p-4 animate-in fade-in duration-300">
      ...
      >
        ...
        <div className="pt-12 pb-8 px-8 flex flex-col items-center w-full relative z-0">
          ...
          <div className="relative group">
            {/* Glow Effect */}
            <div className="absolute -inset-1 bg-gradient-to-tr from-primary via-purple-500 to-indigo-500 rounded-[1.7rem] opacity-75 blur-md group-hover:opacity-100 transition-opacity duration-500" />
