...
export async function GET(
    ...
    try {
        ...
        // Allow bartenders, managers, admins, and general staff
        const allowedRoles = ['bartender', 'manager', 'admin', 'staff', 'chef']; 
        if (!allowedRoles.includes(profile.role)) {
            return NextResponse.json({ error: 'Forbidden: Bartender access only' }, { status: 403 });
        }
      ...
      `)
            ...
            .order('created_at', { ascending: true }) // Oldest first for bar
            .limit(limit);
        ...
        const orders = (data || []).map(o => ({
            ...
            order_items: (o.bar_order_items || []).map((i: any) => ({
                id: i.id,
        ...
        console.error('Bartender Orders Error:', error);
