-- EMERGENCY FIX: RESET MENU RLS TO PERMISSIVE MODE
-- This script drops ALL existing policies and enables a simple "Authenticated Users Can Do Everything" policy.
-- This is to verify if the issue is RLS or something else.
...
-- 2. Ensure columns exist and property_id is nullable
...
-- 4. Create PERMISSIVE Policies (Allow all authenticated users to do anything)
-- We will restrict this later, but we need to unblock the Insert first.
...
-- Menu Categories
CREATE POLICY "allow_all_menu_categories" ON menu_categories
    FOR ALL
    TO authenticated
    USING (true)
    WITH CHECK (true);
...
-- 5. Backfill ownership just in case
